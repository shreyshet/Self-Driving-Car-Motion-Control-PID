...
int main ()
{
  ...
  **/
  PID pid_steer = PID();
  pid_steer.Init(0.3, 0.01, 0.6, 1.2, -1.2); 
  // initialize pid throttle q
  /**
  ...
  **/
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          // Update the delta time with the previous commands
          ...
          **/
           double err_heading = 0.0;
           if (x_points[1]-x_points[0] != 0)
           {
             err_heading = std::atan2((y_points.back()-y_position),(x_points.back()-x_position)) - yaw; 
           }
           error_steer = (.01*(y_points[0]-y_position)+ err_heading);
          ...
          **/
          ...
          error_throttle = (v_points.back() - velocity);// + 0.01*(x_points[0]-x_position);
